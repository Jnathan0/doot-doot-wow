...
class SoundsUtils(commands.Cog):
    ...
    async def sounds(self, ctx):
        ...
        if ctx.message.mentions:
            try:
                ...
                await ctx.send(format_markdown("ERROR: Something broke for this command, maybe it will get fixed."))
                return
        else:
    ...
    async def quicksounds(self, ctx):
        ...
        elif re.match('info', command.split(' ')[1], re.IGNORECASE):
            db = GetDB(config.database_path)
    ...
    async def entrance(self, ctx):
        ...
        try:
            ...
            await ctx.send(format_markdown(e))
            return
